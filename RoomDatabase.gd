...
const ROOM_SCENES := {
	...
	RoomName.TIMER: "res://systems/map/horde_scenes/horde_timer.tscn",
...
const ROOM_SPRITES := {
	...
	RoomName.MINIBOSS: preload("res://map(old)/miniboss.png"),
	RoomName.SHOP: preload("res://map(old)/shop.png"),
	RoomName.ITEM: preload("res://map(old)/item.png"),
	RoomName.TIMER: preload("res://map(old)/time.png"),
	RoomName.DUMMY: preload("res://map(old)/event.png"),
	RoomName.BOSS: preload("res://map(old)/boss.png"),
	RoomName.SECRET_SHOP: preload("res://map(old)/event.png")
}
...
func get_room_name(room_type: int) -> String:
	return ROOM_NAMES.get(room_type, "Unknown")
